...
lambda_identity_matrix = np.identity(conf.d) * conf.lambda_
omega = conf.omega
beta = conf.beta
...
for t in range(1, conf.T+1):
    ...
    for user in range(conf.n):
        V_u = V[Iu[user]]
        bV = np.sum(V_u, axis=0) * omega
        AV = np.matmul(V_u.T, V_u) * (omega - beta)
        U[user] = np.matmul(bV, pinv(SV + AV + conf.m * lambda_identity_matrix))

    # update V
    SU = np.matmul(U.T, U)
    for item in range(conf.m):
        U_i = U[Ui[item]]
        bU = np.sum(U_i, axis=0) * omega
        AU = np.matmul(U_i.T, U_i) * (omega - beta)
        V[item] = np.matmul(bU, pinv(SU + AU + conf.n * lambda_identity_matrix))
